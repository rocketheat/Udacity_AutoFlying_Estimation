...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
  ...
  //if (ekfState(6) < -F_PI) ekfState(6) += 2.f*F_PI;
	...
	// Utilizing the quation from the control lecture on integrating PQR
	// into world frame.
	...
	// Convert body frame gyro input to phi_dot, theta_dot, and psi_dot
	V3F angle_dot = rotMat * gyro;

	// Integrate angle_dot to convert to euler angles
	// Using code similar to the above to update the angles
	...
	// To normalize angle to range [-pi, +pi]
	if (ekfState(6) > M_PI) ekfState(6) -= 2.f * M_PI;
	if (ekfState(6) < -M_PI) ekfState(6) += 2.f * M_PI;
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  // Rotating vector from body frame to inertial frame
  V3F acc_w = attitude.Rotate_BtoI(accel);

  // Using equation 49  from Estimation of Quadrotors to update the transition function to create prediction.
  predictedState(0) = curState(0) + dt * curState(3);
  predictedState(1) = curState(1) + dt * curState(4);
  predictedState(2) = curState(2) + dt * curState(5);
  predictedState(3) = curState(3) + dt * acc_w.x;
  predictedState(4) = curState(4) + dt * acc_w.y;
  predictedState(5) = curState(5) - dt * CONST_GRAVITY + dt * acc_w.z;
...
MatrixXf QuadEstimatorEKF::GetRbgPrime(float roll, float pitch, float yaw)
{
  ...
  // Estimation for Quadrotors equation 52 for RbgPrime
  // For convenience will rename the variables to fit the equation
  ...
  // last row all all zeros Estimation for Quadrotors equation 52
...
void QuadEstimatorEKF::Predict(float dt, V3F accel, V3F gyro)
{
  ...
  // As described in the README the covariance matrix Pk = APk-1AT + Q

  // We need to update the Jacobian with the appropriate data based on equation 51
  // from Estimation of Quadrotors
  gPrime(0, 3) = dt;
  gPrime(1, 4) = dt;
  gPrime(2, 5) = dt;
...
void QuadEstimatorEKF::UpdateFromGPS(V3F pos, V3F vel)
{
  ...
  // updating measurement model using the observations as detailed in equation 54
  // in Estimation for Quadrotors
  zFromX(0) = ekfState(0);
  zFromX(1) = ekfState(1);
  zFromX(2) = ekfState(2);
  zFromX(3) = ekfState(3);
  ...
  // Update hPrime function using equation 55 in Estimation for Quadrotors
  hPrime(0, 0) = 1;
  hPrime(1, 1) = 1;
  hPrime(2, 2) = 1;
  hPrime(3, 3) = 1;
...
void QuadEstimatorEKF::UpdateFromMag(float magYaw)
{
  ...
  // Calculate the difference between estimated and measured yaw angle
  float diff = magYaw - ekfState(6);

  zFromX(0) = ekfState(6);
  // To normalize angle to range [-pi, +pi]
  if (diff > M_PI) zFromX(0) += 2.f * M_PI;
  if (diff < -M_PI) zFromX(0) -= 2.f * M_PI;

  // Updating the h` as noted in in equation 58 in Estimatrion for Quadrotors
  hPrime(0, 6) = 1;
